...
(function() {
  ...
  window.conn.init_connection = function(game_id, user_id, host) {
    ...
    window.conn.channel.bind('update_grid', function(msg) {
      console.log('grid updated');
      window.game.grid = msg.grid;
      return window.game.players = msg.players;
    });
    return window.conn.channel.bind('player_win', function(msg) {
      return window.game.player_win();
    });
  };

  window.conn.update_grid = function(new_grid_elems) {
    return window.conn.dispatcher.trigger('update_grid', {
      new_values: new_grid_elems,
      game_id: window.conn.channel_name
    });
  };

  window.conn.connect_player = function(user_id) {
    return window.conn.dispatcher.trigger('connect_player', {
      user_id: user_id,
      game_id: window.conn.channel_name
    });
  };

  window.conn.disconnect_player = function(user_id) {
    return window.conn.dispatcher.trigger('disconnect_player', {
      user_id: user_id,
      game_id: window.conn.channel_name
    });
  };

  window.conn.pruebas = function(user_id) {
    var channel, dispatcher;
    dispatcher = new WebSocketRails('localhost:3000/websocket');
    channel = dispatcher.subscribe('game_id');
    dispatcher.trigger('new_user', {
      user_id: user_id
    }, window.conn.success, window.conn.failure);
    return channel.bind('new_user', function(msg) {
      $('#pruebas_1').append(msg.user_id);
      $('#pruebas_2').append(msg.list_users[0]);
      return console.log(msg.list_users);
    });
  };
...
(function() {
  ...
  window.game.set_user_id = function(user_id) {
    return window.game.user_id = user_id;
  };

  window.game.canvas_set_size_pc = function(canvas) {
    ...
    return window.game.block_height = Math.floor(window.game.canvas_height / 7);
  };

  window.game.canvas_set_size_touch = function(canvas) {
    var canvas_width, height, table_size, width;
    window.game.canvas = canvas;
    width = 0;
    height = 0;
  ...
  window.game.listener_change_device_orientation = function() {
    return window.addEventListener("orientationchange", (function() {
      if (window.innerHeight > window.innerWidth) {
        $('.game').hide();
        return $('#turn_device_to_landscape_alert').show();
      } else {
        $('.game').show();
        return $('#turn_device_to_landscape_alert').hide();
      }
    }), false);
  };

  window.game.touch_controls = function(controls) {
    controls.show();
    $('.icon-chevron-sign-up').click(function(e) {
      return window.game.key_pressed = 38;
    });
    $('.icon-chevron-sign-right').click(function(e) {
      return window.game.key_pressed = 39;
    });
    $('.icon-chevron-sign-down').click(function(e) {
      return window.game.key_pressed = 40;
    });
    $('.icon-chevron-sign-left').click(function(e) {
      return window.game.key_pressed = 37;
    });
    $('.icon-arrow-up').click(function(e) {
      return window.game.key_pressed = 87;
    });
    $('.icon-arrow-right').click(function(e) {
      return window.game.key_pressed = 68;
    });
    $('.icon-arrow-down').click(function(e) {
      return window.game.key_pressed = 83;
    });
    return $('.icon-arrow-left').click(function(e) {
      return window.game.key_pressed = 65;
    });
  };

  window.game.keyboard_control = function() {
    return $(document).keydown(function(event) {
      event = window.event || event;
      if (((event.keyCode === 68) || (event.keyCode === 87) || (event.keyCode === 83) || (event.keyCode === 65) || (event.keyCode === 37) || (event.keyCode === 38) || (event.keyCode === 39) || (event.keyCode === 40)) && (window.game.key_pressed === null)) {
  ...
  window.game.draw = function(canvas) {
    var b, frames;
    canvas.clearCanvas();
    b = new Date().getTime();
    ...
    if (window.game.grid !== null) {
      return window.game.draw_grid(canvas, window.game.grid, window.game.players);
    }
  };

  window.game.draw_grid = function(canvas, grid, players) {
    var elem, i, j, row, x, y, _i, _j, _len, _len1, _ref, _results;
    x = 0;
    y = 0;
    i = 0;
    j = 0;
    ...
    for (_i = 0, _len = grid.length; _i < _len; _i++) {
      ...
      j = 0;
      x = 0;
      _results.push(y += window.game.block_height);
    }
    return _results;
  };

  window.game.bomb_explosion = function(i, j, grid) {
    var a, b, _i, _j, _len, _len1, _ref, _ref1;
    _ref = [0, 1];
    ...
    return window.game.new_grid_elems = [];
  };

  window.game.bomb_explosion_collision = function(elem, i, j) {
    var e;
    try {
      console.log('collision');
      if (elem.block_type === 'brick' || elem.block_type === 'empty') {
        return window.game.new_grid_elems.push(i + ':' + j + ':' + 'has_explosion' + ':' + new Date().getTime() / 1000);
      } else if (elem.block_type === 'has_player') {
        window.game.new_grid_elems.push(i + ':' + j + ':' + 'has_explosion' + ':' + new Date().getTime() / 1000);
        return window.game.player_lose();
      }
    } catch (_error) {
      e = _error;
  ...
  window.game.draw_explosion = function(canvas, x, y) {
    return canvas.drawPolygon({
      fillStyle: "#36c",
  ...
  window.game.draw_number = function(canvas, x, y, i) {
  ...
  window.game.draw_brick = function(canvas, x, y) {
  ...
  window.game.draw_empty = function(canvas, x, y) {
    return canvas.drawRect({
      strokeStyle: "#AEB8BA",
      strokeWidth: 1,
      fillStyle: "#FFFFFF",
  ...
  window.game.draw_has_bomb = function(canvas, x, y, bomb_color) {
    var height, width;
  ...
  window.game.draw_block = function(canvas, x, y) {
    return canvas.drawRect({
      strokeStyle: "#AEB8BA",
      strokeWidth: 1,
      fillStyle: "#000000",
  ...
  window.game.draw_player = function(canvas, x, y, head_color, eyes_color, body_color, limbs_color) {
    var height, width;
  ...
  window.game.player_actual_position = function(grid) {
    var elem, i, j, row, _i, _j, _len, _len1;
    i = -1;
    for (_i = 0, _len = grid.length; _i < _len; _i++) {
      row = grid[_i];
      i++;
      ...
      for (_j = 0, _len1 = row.length; _j < _len1; _j++) {
        elem = row[_j];
        if ((elem.block_type === 'has_player') && (elem.user_id === window.game.user_id)) {
          return [i, j];
        }
        j++;
      }
  ...
  window.game.place_bomb = function(bomb_position, grid) {
    var actual_position, future_position;
    ...
    if ((future_position[0] >= 0) && (future_position[0] <= 6) && (future_position[1] >= 0) && (future_position[1] <= 8)) {
      if (grid[future_position[0]][future_position[1]].block_type === 'empty') {
        ...
        return console.log('bomb_placed');
      }
    }
  };

  window.game.player_mov_collision = function(i, j, grid) {
    var back;
    back = 'ok';
    if ((j < 0) || (j > 8) || (i < 0) || (i > 6)) {
      back = 'collision';
  ...
  window.game.disconnect_player = function() {
    var old_position;
    old_position = window.game.player_actual_position(window.game.grid);
    return window.conn.update_grid([old_position[0] + ':' + old_position[1] + ':' + 'empty' + ':' + 0]);
  };

  window.game.player_lose = function() {
    $('#modal_window_game_win_lose').find('#text_message').html('Game finish');
    return $('#trigger_finish_modal').click();
  };

  window.game.player_win = function() {
    $('#modal_window_game_win_lose').find('#text_message').html('Game finish');
    return $('#trigger_finish_modal').click();
  };
...
jQuery.fn.colourPicker = function (conf) {
    ...
    var config = jQuery.extend({
        ...
        ico:        '/assets/color_picker_image.gif',                // SRC to colour-picker icon
...
(function() {
  window.user_profile_costomize_bomb = function(canvas, bomb_color, explosion_color, width, height) {
    var x, y;
    canvas.clearCanvas();
    x = 65;
    y = 85;
  ...
  window.user_profile_costomize_bombardier = function(canvas, head_color, body_color, limbs_color, eyes_color, width, height) {
    var x, y;
    canvas.clearCanvas();
    x = 65;
    y = 25;
